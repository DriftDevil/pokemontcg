...
export async function GET(request: NextRequest, { params }: { params: { userID: string } }) {
  const { userID } = params;
  const CONTEXT = `API /admin/users/${userID}/collection/cards`;
  ...
  logger.info(CONTEXT, `Forwarding get collection for user ${userID} request to external API: ${externalUrl}`);
