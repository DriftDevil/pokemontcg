...
export async function GET(request: NextRequest, { params }: { params: { userID: string } }) {
  ...
  if (!EXTERNAL_API_BASE_URL) {
    console.error(`[API /admin/users/${userID}/collection/cards] External API base URL not configured.`);
  ...
  const externalUrl = `${EXTERNAL_API_BASE_URL}/user/admin/${userID}/collection/cards`;
  console.log(`[API ${request.nextUrl.pathname}] Forwarding get collection for user ${userID} request to external API: ${externalUrl}`);
