...
export async function GET(request: NextRequest) {
  ...
  const externalUrl = `${EXTERNAL_API_BASE_URL}/sets${searchParams ? `?${searchParams}` : ''}`;

  try {
    const response = await fetch(externalUrl, {
      headers: {
        // Add any necessary headers for the external API, if an API key were needed, it would go here.
